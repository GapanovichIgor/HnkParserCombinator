module HnkParserCombinator.Composition
...
let (>>.) (p1: Parser<'i, 's, _, _>) (p2: Parser<'i, 's, 'e, 'o>) : Parser<'i, 's, 'e, 'o> = combine snd p1 p2

let (.>>) (p1: Parser<'i, 's, 'e, 'o>) (p2: Parser<'i, 's, _, _>) : Parser<'i, 's, 'e, 'o> = combine fst p1 p2
...
let zeroOrMoreDelimited
    (delimiter: Parser<'i, 's, _, _>)
    (parser: Parser<'i, 's, 'e, 'o>)
    : Parser<'i, 's, 'e, 'o list> =
    fun (tape, state) ->
        let mutable keepGoing = true
            ...
            [ while keepGoing do
                  ...
                  | Ok o ->
                      ...
                      | Ok o ->
                          ...
                          state <- o.state
                      | _ -> keepGoing <- false
                  | Error e -> if moreRequired then error <- Some e ]
